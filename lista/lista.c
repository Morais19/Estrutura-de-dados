...
Lista * CriarLista(int C) {
    ...
    if(L == NULL) {
        ...
        printf("ERRO: nao ha memoria para a estrutura da lista!\n");
        return NULL;
    }
    ...
    if(L ->Dados == NULL) {
        ...
        printf("ERRO: nao ha memoria para armazenar o vetor!\n");
        free(L);
        return NULL;
    }
    ...
    return L; // Retornando o ponteiro (endereço da struct Lista)
}

void AdicionarTarefa(Lista * L, Tarefa T) {

    if (L == NULL) return;
    if (L->Tamanho == L->Capacidade)return; // Lista está completa
    L->Dados[L->Tamanho] = T;
    L->Tamanho++;
